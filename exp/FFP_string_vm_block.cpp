...
/*
vector<bool> integer_to_bool_vector(unsigned long &bits_per_alphabet, int int_size)
{
    vector<bool> bool_vector;
    bool_vector.reserve(bits_per_alphabet);

    for (int cy1=bits_per_alphabet-1; cy1!=-1; cy1--) //0 base
    {
    ...
    //print_vector_bool_readable(bool_vector);
...
/*
bool key_hash_check(char &read_char, sparse_hash_map<char, string, hash<char>, compare_char> &key_reg_hash)
{

    for (sparse_hash_map<char, string, hash<char>, compare_char>::iterator it=key_reg_hash.begin(); it!=key_reg_hash.end(); ++it)
    {
    ...
    sparse_hash_map<char, string, hash<char>, compare_char> &key_reg_hash)
{
    ...
    key_bit_string.resize(bits_per_feature, '0');
...
void multi_hash_container_output(sparse_hash_map<  vector<bool>, sparse_hash_map< vector<bool>,  long long, hash< vector<bool> >, compare_bool_vector>, hash< vector<bool> >, compare_bool_vector>  &prim_index_hash, deque<string> &max_index_key_deque,
                                 sparse_hash_map<string, long long, hash<string>, compare_string> &compress_text_hash, bool ratio_flag, int str_key_frag_size, double &feature_hit_cnt, stringstream &output_stream)
{
    ...
    vector< vector<bool> > sub_index_vector;
...
int read_sliding_process(stringstream &read_f, int feature_length, bool backward_flag, double &feature_hit_cnt, bool unmasked_treat_flag, bool ratio_output_flag, bool ry_code_flag,
    ...
    vector<string> &max_index_key_vector, unsigned long &bits_per_feature, string &deleted_hash_key)
{
...
unsigned long count_bits_per_alphabet(string &key_str, int additional_bit)
{
    unsigned long bits_per_alphabet=0;

    while(key_str.length() > pow(2, bits_per_alphabet))
    {
        bits_per_alphabet++;

    }

    return (bits_per_alphabet+additional_bit);
...
int main(int argc, char** argv)
{
    ...
    unsigned long bits_per_alphabet = count_bits_per_alphabet(key_str, additional_bit);
    unsigned long bits_per_feature = (unsigned long)(ceil(((double)(bits_per_alphabet*feature_length) / 8)) * 8);

    sparse_hash_map<char, string, hash<char>, compare_char> key_reg_hash;
    key_hash_register(key_str, key_reg_hash, bits_per_alphabet);
        ...
        } else
        {
            ...
            while (1)
            {
                ...
                for (vector<string>::iterator it=str_block_vector.begin(); it!=str_block_vector.end(); ++it)
                {
                    ...
                    read_sliding_process(read_str_f, feature_length, backward_flag, feature_hit_cnt, unmasked_treat_flag, ratio_output_flag,
                                        ry_code_flag, key_reg_hash, prim_index_hash, max_index_key_vector, bits_per_feature, deleted_hash_key);
