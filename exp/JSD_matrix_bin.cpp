...
{
    /*
    char key_buf[256]; ///is necessary to recover original string
    double key_value=0.0;
    */

    if (bytes_per_feature==0 && bytes_per_value==0) //because they can't be 0 while run
    {
        bytes_per_feature = t_stream.get() + 0; //convert to numeric, read one byte
        bytes_per_value = t_stream.get() + 0; //convert to numeric, read one byte
    ...
    /*
    if (t_stream.read(key_buf, bytes_per_feature))
    {
        //cout << "key" << "\t" << t_stream.tellg() << endl;
        n_key = string(key_buf);

        t_stream.read(reinterpret_cast<char*>(&key_value), bytes_per_value);
        n_value = key_value;

        //cout << "value" << "\t" << t_stream.tellg() << endl;
        if (t_stream.tellg()==t_stream.str().size())
        {
            cout << "reach EOF" << endl;
...
double jsd_distance_key_sort(string p_path, string q_f_buf, int delimiter_int)
{
    ...
    */
    ...
    //stringstream read_p_f(p_f_buf, ios::in|ios::out|ios::binary);
    ...
    //cout << "p\t" << read_p_f.str().size() << endl;
    //cout << "q\t" << read_q_f.str().size() << endl;

    //while (p_read_flag!=false || q_read_flag!=false)
    while (!read_p_f.eof() || !read_q_f.eof())
    {
        //if (p_value==0.0 && !read_p_f.eof())
        ...
        /*
        if (p_value < 0 || q_value < 0 || p_value > 1 || q_value > 1) //logic error proof
        {
        ...
        */
        ...
        //printf("++ p: %s:%e \t q: %s:%e\n", p_key.c_str(), p_value, q_key.c_str(), q_value);
        //printf("P; bp_feature= %d, bp_value= %d\n", p_bytes_per_feature, q_bytes_per_value);
        //printf("Q; bp_feature= %d, bp_value= %d\n", q_bytes_per_feature, q_bytes_per_value);
        /*
        ...
        } else if (p_key==q_key && p_value!=0 && q_value!=0) //case when p_key==q_key but one of key is 0 creat 'nana'
        {
            //printf("++ p: %s\t%e \t q: %s\t%e\n", p_key.c_str(), p_value, q_key.c_str(), q_value);
            //printf("++ p: %e \t q: %e\n", p_value, q_value);
        ...
        } else //what this can be? this case shouldn't be exists
        {
            cout << "pass cases" << endl;
            printf("p: %s\t%e \t q: %s\t%e\n", p_key.c_str(), p_value, q_key.c_str(), q_value);
        ...
        /*
        if (std::isnan(Hp) || std::isnan(Hq) || std::isnan(Hm)) //logic error proof
        {
            printf("p: %s\t%e \t q: %s\t%e\n", p_key.c_str(), p_value, q_key.c_str(), q_value);
            //cout << Hp << "\t" << Hq << "\t" << Hm << endl;
            printf("Hp: %f, Hq: %f, Hm: %f\n", Hp, Hq, Hm);
        ...
        */
        ...
        //cout << "P" << "\t" << p_key << "\t" << p_value << endl;
        //cout << "Q" << "\t" << q_key << "\t" << q_value << endl;
        //printf("p: %s:%e \t q: %s:%e\n", p_key.c_str(), p_value, q_key.c_str(), q_value);
