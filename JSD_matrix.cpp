...
void print_value_vector_str(stringstream &output_stream, vector< vector<double> > &fut_value_vector, vector<string> &load_path_vector, int start_item_n)
{
    //int stream_size;
    ...
    if (start_item_n==0)
    {
        ret_stream_size = snprintf(stream_buf, sizeof(stream_buf), "%d\n", (int)load_path_vector.size());
    ...
    for (vector< vector<double> >::size_type r_it=start_item_n; r_it<load_path_vector.size(); r_it++)
    {
        ...
        ret_stream_size = snprintf(stream_buf, sizeof(stream_buf), "%-10s", base_name_str.c_str());
        ...
        {
            ret_stream_size = snprintf(stream_buf, sizeof(stream_buf), "%.4e\t", fut_value_vector[r_it][c_it]);
            output_stream << stream_buf;
            //continue;
        }


        ret_stream_size = snprintf(stream_buf, sizeof(stream_buf), "%.4e\n", 0.0);
        output_stream << stream_buf;
...
int main(int argc, char** argv) //in case of output JSD distance(incomplete)
{
    ...
    while ((opt = getopt(argc, argv, "hc:t:r:v")) !=EOF) // EOF = -1
    {
        switch (opt)
        {
            case 'h':
    ...
    if (argc - optind > 1)
    {
        ...
        } else
        {
            ...
            print_value_vector_str(output_stream, fut_value_vector, load_path_vector, start_item_n); //print out and refresh all variable
