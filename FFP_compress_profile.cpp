...
void multi_hash_container_input_rev(sparse_hash_map<string, sparse_hash_map<string, long long, hash<string>, compare_string>, hash<string>, compare_string>  &prim_index_hash, deque<string> &max_index_key_deque, string str_key)
{
                                 ...
                                 sparse_hash_map<string, long long, hash<string>, compare_string> &compress_text_hash, bool ratio_flag, int str_key_frag_size, double &feature_hit_cnt, stringstream &output_stream)
{
    ...
    for (deque<string>::iterator it=max_index_key_deque.begin(); it!=max_index_key_deque.end(); ++it)
    {
        ...
        for (vector<string>::iterator vector_it=sub_index_vector.begin(); vector_it!=sub_index_vector.end(); ++vector_it)
        {
            ...
            if (ratio_flag==true)
            {
                stream_size = snprintf(NULL, 0, "%s|%.4e\n", (*vector_it).c_str(), (double)(prim_index_hash[*it])[*vector_it] / feature_hit_cnt);
                stream_buf = (char*)realloc(stream_buf, (stream_size+1)*sizeof(char));
                snprintf(stream_buf, sizeof(stream_buf), "%s|%.4e\n", (*vector_it).c_str(), (double)(prim_index_hash[*it])[*vector_it] / feature_hit_cnt);

            } else
            {
                stream_size = snprintf(NULL, 0, "%s|%lld\n", (*vector_it).c_str(), (prim_index_hash[*it])[*vector_it]);
                stream_buf = (char*)realloc(stream_buf, (stream_size+1)*sizeof(char));
                snprintf(stream_buf, sizeof(stream_buf), "%s|%lld\n", (*vector_it).c_str(), (prim_index_hash[*it])[*vector_it]);
                            ...
                            double &feature_hit_cnt, int str_key_frag_size, stringstream &output_stream)
{
    ...
    for (sparse_hash_map<string, sparse_hash_map<string, long long, hash<string>, compare_string>, hash<string>, compare_string>::iterator it=prim_index_hash.begin(); it!=prim_index_hash.end(); ++it)
    {
        for (sparse_hash_map<string, long long, hash<string>, compare_string>::iterator sub_it=(it->second).begin(); sub_it!=(it->second).end(); ++sub_it)
        {
            ...
            if (ratio_flag==true)
            {
                ret_stream_size=snprintf(NULL, 0, "%.4e", (double)(sub_it->second) / feature_hit_cnt);
                stream_buf = (char*)realloc(stream_buf, (ret_stream_size+1)*sizeof(char));
                snprintf(stream_buf, sizeof(stream_buf), "%.4e", (double)(sub_it->second) / feature_hit_cnt);

            } else //convert longlong to string type
            {
                ret_stream_size=snprintf(NULL, 0, "%lld", sub_it->second);
                stream_buf = (char*)realloc(stream_buf, (ret_stream_size+1)*sizeof(char));
                snprintf(stream_buf, sizeof(stream_buf), "%lld", sub_it->second);
    ...
    for (sparse_hash_map<string, long long, hash<string>, compare_string>::iterator it=compress_text_hash.begin(); it!=compress_text_hash.end(); it++)
    {
        ...
        ret_stream_size=snprintf(NULL, 0, "#%s|%lld\n", (it->first).c_str(), order_cnt);
        stream_buf = (char*)realloc(stream_buf, (ret_stream_size+1)*sizeof(char));
...
int read_sliding_process(stringstream &read_f, int feature_length, bool backward_flag, double &feature_hit_cnt, bool part_output_flag, bool unmasked_treat_flag, bool ratio_output_flag, bool ry_code_flag,
                            ...
                            deque<string> &max_index_key_deque)
{
    ...
    while (read_f.get(read_char))
    {
        if (read_char=='>' || read_f.eof())
        {
            ...
            if (part_output_flag==true && !prim_index_hash.empty())
            {
                //multi_hash_container_output(prim_index_hash, ratio_output_flag, sub_container_size_limit, feature_hit_cnt);
            ...
            if (key_hash_check(read_char, key_reg_hash)!=-1) //char registered in key_reg_hash
            {
                ...
                if (forward_index_str.length() == feature_length)
                {
                    ...
                    if (backward_flag==true)
                    {
                        ...
                        {
                            multi_hash_container_input_rev(prim_index_hash, max_index_key_deque, backward_index_str);

                        } else //backward_index_str larger or equal
                        {
                            multi_hash_container_input_rev(prim_index_hash, max_index_key_deque, forward_index_str);
...
void show_help()
{
    ...
    cout << "-m, partial output, split by fasta header '>'" << endl;
...
int main(int argc, char** argv)
{
    ...
    while ((opt = getopt(argc, argv, "hvs:e:ack:rmnub:t:V")) !=EOF) // EOF = -1
    {
        switch (opt)
        {
            case 'h':
    ...
    if (argc  - optind == 2) //direct file write version; load_path and save_path
    {
        ...
        } else
        {
            ...
            {
                //prim_index_hash.resize(2); //? pre-resize saves time

                if (read_sliding_process(read_str_f, feature_length, backward_flag, feature_hit_cnt, part_output_flag, unmasked_treat_flag, ratio_output_flag, ry_code_flag,
                    key_reg_hash, prim_index_hash, max_index_key_deque)==1)
                {
                    if (max_vocab_find_flag==true)
                    {
                        recent_vocab_size = vocab_size_measure(prim_index_hash, bottom_limit, top_limit);

                        ret_stream_size = snprintf(NULL, 0, "l-mer; %d, vocab_size; %lld\n", feature_length, recent_vocab_size); //find string(c) size
                        ///formated size larger than given stream_buff?
                        stream_buf = (char*)realloc(stream_buf, (ret_stream_size + 1)*sizeof(char)); //+1 at the end(/0)
                        snprintf(stream_buf, sizeof(stream_buf), "l-mer; %d, vocab_size; %lld\n", feature_length, recent_vocab_size); //format char*
                        ...
                        } else
                        {
                            ...
                            ret_stream_size =  snprintf(NULL, 0, "#Point of MAX vocabulary size(%lld <= frequency <= %lld); %d\n", bottom_limit, top_limit, feature_length-1);
                            ///formated size larger than given stream_buff?
                            stream_buf = (char*)realloc(stream_buf, (ret_stream_size + 1)*sizeof(char)); //+1 at the end(/0)
                            snprintf(stream_buf, sizeof(stream_buf),"#Point of MAX vocabulary size(%lld <= frequency <= %lld); %d\n", bottom_limit, top_limit, feature_length-1);
